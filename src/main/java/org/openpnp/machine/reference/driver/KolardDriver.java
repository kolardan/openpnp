...
public class KolardDriver extends AbstractTelnetDriver implements Runnable {
    private static final Logger logger = LoggerFactory.getLogger(KolardDriver.class);
    ...
    protected List<String> sendCommand(String command, long timeout) throws Exception {
        synchronized (commandLock) {
            if (command != null) {
                ...
                outputSocket.writeBytes(command);
                outputSocket.writeBytes("\n");
